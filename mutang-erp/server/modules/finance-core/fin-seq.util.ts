...
export const MAX_SEQ_RETRY: number = 8;
const SEQ_NO_LENGTH: number = 3;
const UNIQUE_VIOLATION_CODE: string = '23505';

/** 财务编号前缀：收款 SK、付款 FK、成本 CB、结算 JS、发票 FP */
export const RECEIPT_NO_PREFIX: string = 'SK';
export const PAYMENT_NO_PREFIX: string = 'FK';
export const COST_NO_PREFIX: string = 'CB';
export const SETTLEMENT_NO_PREFIX: string = 'JS';
export const INVOICE_NO_PREFIX: string = 'FP';
...
/** 财务编号 = 前缀 + 完整年月(6位) + 3位序号，如 SK202609001 */
...
/** 生成候选编号：前缀 + 完整年月(6位) + 3位序号，如 SK202609001 */
...
/** 单条插入：编号冲突(23505)时重查已有序号+1重试（线性递增兕底），最多 8 次 */
export async function insertWithSeqNo<T extends { id: number }>(opts: {
  db: PostgresJsDatabase;
  table: PgTable;
  noColumn: Column;
  prefix: string;
  insert: (no: string) => Promise<T[]>;
}): Promise<{ id: number; row: T; no: string }> {
  const yearMonth: string = buildYearMonth();
  let seq: number =
  ...
  for (let attempt: number = 0; attempt < MAX_SEQ_RETRY; attempt += 1) {
    ...
    try {
      const inserted: T[] = await opts.insert(no);
      if (inserted.length === 0) {
        throw new BadRequestException('创建失败');
      }
      return { id: inserted[0].id, row: inserted[0], no };
    } catch (error: unknown) {
...
export async function batchInsertWithSeqNo<V>(opts: {
  ...
  insert: (values: V[]) => Promise<{ id: number }[]>;
}): Promise<number> {
  const yearMonth: string = buildYearMonth();
  let baseSeq: number =
    (await countExistingSeq(
  ...
  for (let attempt: number = 0; attempt < MAX_SEQ_RETRY; attempt += 1) {
    ...
    try {
      const inserted: { id: number }[] = await opts.insert(
